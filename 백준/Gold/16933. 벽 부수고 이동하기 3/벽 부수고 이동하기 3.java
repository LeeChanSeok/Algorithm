		...
		while(!q.isEmpty()) {
			...
			while(qSize-- > 0){
				...
				for(int d = 0; d < 4; ++d) {
					...
					// '0'으로 갈 때
					if(board[nx][ny] == '0') {
						if(cur.k > visit[nx][ny][night]) {
							visit[nx][ny][night] = cur.k;
					...
					if((night&1) == 1) {
						//제자리
						if(cur.k > visit[cur.x][cur.y][night]) {
							visit[cur.x][cur.y][night] = cur.k;
							q.add(new Point(cur.x, cur.y, cur.k));
						}
					}else {//낮일때
						// '0'으로 갈 때
						if(board[nx][ny] == '1') {
							 // '1' 으로 갈 때
							if(cur.k-1 > visit[nx][ny][night]){
								visit[nx][ny][night] = cur.k-1;
